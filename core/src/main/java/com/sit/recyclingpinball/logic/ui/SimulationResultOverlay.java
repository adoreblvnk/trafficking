...
public class SimulationResultOverlay extends AbstractScene implements InputListener {
    ...
    public void create() {
        ...
        if (isWin) {
            getAudio().playSound(LogicConstants.SOUND_WIN, LogicConstants.VOLUME_DEFAULT);
        } else {
            getAudio().playSound(LogicConstants.SOUND_LOSE, LogicConstants.VOLUME_DEFAULT);
        }
    }
    ...
    public void render() {
        ...
        for (int i = 0; i < totalTrash; i++) {
            ...
            if (i < score) {
                getGraphics().drawTexture(LogicConstants.TEX_STAR, starX, starsY, 64, 60);
            } else {
                getGraphics().fillRectangle(starX + 16, starsY + 14, 32, 32, 0.3f, 0.3f, 0.3f, 0.4f);
            }
        }

        // Score text (white on dark overlay — no button behind this)
        getGraphics().setTextColor(LogicConstants.COLOR_TEXT_LIGHT[0], LogicConstants.COLOR_TEXT_LIGHT[1],
